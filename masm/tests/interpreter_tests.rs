use masm::{assembler::assemble_to_masi, disassembler::load as load_masi, interpreter::run_masi_with_io};
...
fn run_asm_with_io(asm: &str, input: &str) -> (String, String) {
	...
	let _state = run_masi_with_io(&masi, &mut out_buf, &mut err_buf, Some(&mut input_reader)).expect("run");
...
fn test_syscall_write_and_read() {
	...
	if let Some("") = lines.last().copied() { lines.pop(); }
