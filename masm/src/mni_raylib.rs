...
fn parse_u64(s: &str) -> Option<u64> { s.trim().parse::<u64>().ok() }
fn parse_i32(s: &str) -> Option<i32> { s.trim().parse::<i32>().ok() }
...
fn reg_value_by_name(ctx: &MniCtx, name: &str) -> Option<u64> {
    ...
    nm.get(key.as_str()).and_then(|id| ctx.state.regs.get(id).copied())
}

fn parse_u64_arg(ctx: &MniCtx, s: &str) -> Option<u64> {
    let st = s.trim();
    if let Ok(v) = st.parse::<u64>() { return Some(v); }
    if let Some(hex) = st.strip_prefix("0x").or_else(|| st.strip_prefix("0X")) {
        if let Ok(v) = u64::from_str_radix(hex, 16) { return Some(v); }
    }
    reg_value_by_name(ctx, st)
}

fn parse_usize_arg(ctx: &MniCtx, s: &str) -> Option<usize> { parse_u64_arg(ctx, s).map(|v| v as usize) }
fn parse_i32_arg(ctx: &MniCtx, s: &str) -> Option<i32> {
    if let Ok(v) = s.trim().parse::<i32>() { return Some(v); }
    parse_u64_arg(ctx, s).map(|v| v as i32)
}
fn parse_usize_lit(s: &str) -> Option<usize> { s.trim().parse::<usize>().ok() }
fn parse_u8_lit(s: &str) -> Option<u8> { s.trim().parse::<u8>().ok() }
...
pub fn register_raylib_mni(reg: &mut ModuleRegistry) {
    ...
    reg.register("Raylib", "draw_text", |ctx: &mut MniCtx| {
        if ctx.args.len() < 9 { return; }
        let x = ctx.args.get(0).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let y = ctx.args.get(1).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let size = ctx.args.get(2).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(20);
        ...
        let addr = ctx.args.get(7).and_then(|s| parse_usize_arg(ctx, s)).unwrap_or(0);
        let len = ctx.args.get(8).and_then(|s| parse_usize_arg(ctx, s)).map(|v| v as usize).unwrap_or(0);
        let mem = ctx.state.memory.lock().unwrap();
        let end = addr.saturating_add(len).min(mem.len());
        ...
        RL_STATE.with(|cell| {
            if let Some(gl) = cell.borrow_mut().as_mut() {
                gl.commands.push(DrawCommand::Text(text, x, y, size, r, g, b, a));
            }
        });
    });
    ...
    reg.register("Raylib", "draw_rectangle", |ctx: &mut MniCtx| {
        if ctx.args.len() < 8 { return; }
        let x = ctx.args.get(0).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let y = ctx.args.get(1).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let w = ctx.args.get(2).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let h = ctx.args.get(3).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        ...
        RL_STATE.with(|cell| {
            if let Some(gl) = cell.borrow_mut().as_mut() {
                gl.commands.push(DrawCommand::Rectangle(x, y, w, h, r, g, b, a));
            }
        });
    });
    ...
    reg.register("Raylib", "draw_rectangle_lines", |ctx: &mut MniCtx| {
        ...
        let w = ctx.args.get(2).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let h = ctx.args.get(3).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let thickness = ctx.args.get(4).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(1);
        ...
        RL_STATE.with(|cell| {
            if let Some(gl) = cell.borrow_mut().as_mut() {
                gl.commands.push(DrawCommand::RectangleLines(x, y, w, h, thickness, r, g, b, a));
            }
        });
    });
    ...
    reg.register("Raylib", "draw_circle", |ctx: &mut MniCtx| {
        if ctx.args.len() < 7 { return; }
        let x = ctx.args.get(0).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let y = ctx.args.get(1).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let radius = ctx.args.get(2).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let r = ctx.args.get(3).and_then(|s| parse_u8_lit(s)).unwrap_or(255);
        let g = ctx.args.get(4).and_then(|s| parse_u8_lit(s)).unwrap_or(255);
        let b = ctx.args.get(5).and_then(|s| parse_u8_lit(s)).unwrap_or(255);
        let a = ctx.args.get(6).and_then(|s| parse_u8_lit(s)).unwrap_or(255);
        RL_STATE.with(|cell| {
            if let Some(gl) = cell.borrow_mut().as_mut() {
                gl.commands.push(DrawCommand::Circle(x, y, radius, r, g, b, a));
            }
        });
    });
    ...
    reg.register("Raylib", "draw_line", |ctx: &mut MniCtx| {
        if ctx.args.len() < 8 { return; }
        let x1 = ctx.args.get(0).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let y1 = ctx.args.get(1).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let x2 = ctx.args.get(2).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let y2 = ctx.args.get(3).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let r = ctx.args.get(4).and_then(|s| parse_u8_lit(s)).unwrap_or(255);
        let g = ctx.args.get(5).and_then(|s| parse_u8_lit(s)).unwrap_or(255);
        let b = ctx.args.get(6).and_then(|s| parse_u8_lit(s)).unwrap_or(255);
        let a = ctx.args.get(7).and_then(|s| parse_u8_lit(s)).unwrap_or(255);
        RL_STATE.with(|cell| {
            if let Some(gl) = cell.borrow_mut().as_mut() {
                gl.commands.push(DrawCommand::Line(x1, y1, x2, y2, r, g, b, a));
            }
        });
    });
    ...
    let w = ctx.args.get(0).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(640);
    let h = ctx.args.get(1).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(480);
        let title = ctx.args.get(2).map(|s| s.as_str()).unwrap_or("masm");
        // Print all arguments for debugging
        println!("[MNI] Raylib.init args: {:?} (w={}, h={}, title={})", ctx.args, w, h, title);
        RL_STATE.with(|cell| {
            let mut slot = cell.borrow_mut();
            if slot.is_some() { *slot = None; }
    ...
    reg.register("Raylib", "set_target_fps", |ctx: &mut MniCtx| {
        let fps = ctx.args.get(0)
            .and_then(|s| s.trim().parse::<u32>().ok())
            .or_else(|| parse_u64_arg(ctx, ctx.args.get(0).map(String::as_str).unwrap_or(""))
                .map(|v| v as u32))
            .unwrap_or(60);
        RL_STATE.with(|cell| {
            if let Some(gl) = cell.borrow_mut().as_mut() { gl.rl.set_target_fps(fps); }
        });
    });
    ...
    reg.register("Raylib", "is_key_down", |ctx: &mut MniCtx| {
        let code = ctx.args.get(0)
            .and_then(|s| s.trim().parse::<i32>().ok())
            .or_else(|| parse_u64_arg(ctx, ctx.args.get(0).map(String::as_str).unwrap_or("")).map(|v| v as i32))
            .unwrap_or(0);
    ...
    reg.register("Raylib", "present", |ctx: &mut MniCtx| {
        let addr = ctx.args.get(0).and_then(|s| parse_usize_arg(ctx, s)).unwrap_or(0);
        let w = ctx.args.get(1).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        let h = ctx.args.get(2).and_then(|s| parse_i32_arg(ctx, s)).unwrap_or(0);
        if w <= 0 || h <= 0 { return; }

        let pitch = ctx.args.get(3).and_then(|s| parse_usize_lit(s)).unwrap_or((w as usize) * 4);
                ...
                for cmd in gl.commands.drain(..) {
                    match cmd {
                        ...
                        DrawCommand::RectangleLines(x, y, w, h, thickness, r, g, b, a) => {
                            ...
                            d.draw_rectangle(x, y + h - thickness as i32, w, thickness as i32, Color::new(r, g, b, a)); // bottom
                            d.draw_rectangle(x, y, thickness as i32, h, Color::new(r, g, b, a)); // left
                            d.draw_rectangle(x + w - thickness as i32, y, thickness as i32, h, Color::new(r, g, b, a)); // right
                        }
    ...
    reg.register("Raylib", "close", |_ctx: &mut MniCtx| {
        RL_STATE.with(|cell| {
            let mut slot = cell.borrow_mut();
            if let Some(gl) = slot.take() { drop(gl); }
        });
    });
    ...
    reg.register("Raylib", "set_pixel", |ctx: &mut MniCtx| {
        if ctx.args.len() < 8 { return; }
        let base = ctx.args.get(0).and_then(|s| parse_usize_arg(ctx, s)).unwrap_or(0);
        let w = ctx.args.get(1).and_then(|s| parse_usize_arg(ctx, s)).unwrap_or(0);
        let x = ctx.args.get(2).and_then(|s| parse_usize_arg(ctx, s)).unwrap_or(0);
        let y = ctx.args.get(3).and_then(|s| parse_usize_arg(ctx, s)).unwrap_or(0);
        ...
        if mem.len() < need { mem.resize(need, 0); }
        if need <= mem.len() {
            mem[idx] = r; mem[idx + 1] = g; mem[idx + 2] = b; mem[idx + 3] = a;
        }
    });
    ...
    let base = ctx.args.get(0).and_then(|s| parse_usize_arg(ctx, s)).unwrap_or(0);
    let w = ctx.args.get(1).and_then(|s| parse_usize_arg(ctx, s)).unwrap_or(0);
    let h = ctx.args.get(2).and_then(|s| parse_usize_arg(ctx, s)).unwrap_or(0);
