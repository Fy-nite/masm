...
fn parse_data_line(line: &str, data: &mut DataSections) -> bool {
    ...
    // Only treat a leading "label:" as a label definition; ignore ':' inside strings or after whitespace
    {
        ...
        for (i, ch) in t.char_indices() {
            match ch {
                '"' => { in_str = !in_str; }
                ':' if !in_str => { idx_colon = Some(i); break; }
                ' ' | '\t' => { if !in_str { break; } }
                _ => {}
            }
        }
            ...
            if let Some(start) = after_name.find('<') {
                if let Some(end) = after_name[start+1..].find('>') {
                    ...
                    match ty.to_ascii_uppercase().as_str() {
                        "BYTE" => {
                            if let Ok(v) = value_str.parse::<i64>() { data.directives.push(DataDirective::Db(Some(name.to_string()), vec![v as u8])); return true; }
                        }
                        "WORD" => {
                            if let Ok(v) = value_str.parse::<i64>() { data.directives.push(DataDirective::Dw(Some(name.to_string()), vec![v as u16])); return true; }
                        }
                        "DWORD" => {
                        ...
                        "DOUBLE" => {
                            if let Ok(v) = value_str.parse::<f64>() { data.directives.push(DataDirective::Ddbl(Some(name.to_string()), vec![v])); return true; }
                        }
                        _ => {}
                    }
                }
...
fn expand_macros(src: &str) -> String {
    ...
    while i < lines.len() {
        ...
        if trimmed.to_lowercase().starts_with("macro ") {
            ...
            let params: Vec<String> = parts.map(|s| s.trim_matches(&[','][..]).to_string()).filter(|s| !s.is_empty()).collect();
            i += 1; let mut body: Vec<String> = Vec::new();
            while i < lines.len() {
                let mut ln = lines[i].clone();
                if let Some(idx) = ln.find(';') { ln.truncate(idx); }
                if ln.trim().eq_ignore_ascii_case("endmacro") { break; }
                body.push(lines[i].clone());
                i += 1;
            }
            // Skip ENDMACRO line
            while i < lines.len() {
                if lines[i].trim().eq_ignore_ascii_case("endmacro") { break; }
                i += 1;
            }
            if i < lines.len() { i += 1; }
...
fn expand_includes(src: &str, base_dir: Option<&Path>) -> Result<String, String> {
    ...
    fn resolve_path(p: &str, base: Option<&Path>) -> PathBuf {
        ...
        if let Some(b) = base { return b.join(path); }
        if let Ok(cwd) = std::env::current_dir() { return cwd.join(path); }
        path.to_path_buf()
    }
...
fn parse_instructions(src: &str, base_dir: Option<&Path>) -> (Vec<Instruction>, DataSections) {
    ...
    for raw in src.lines() {
        ...
        if lower.starts_with("#include_bytes ") || lower.starts_with("#include_dq ") {
            ...
            let rest = if is_dq { &trimmed[12..] } else { &trimmed[15..] };
            // Expect: label, "path"
            let mut parts = rest.splitn(2, ',');
            ...
            let base = base_dir.and_then(|p| Some(p.to_path_buf())).or_else(|| std::env::current_dir().ok()).unwrap_or_else(|| PathBuf::from("."));
            let abs = if Path::new(path_str).is_absolute() { PathBuf::from(path_str) } else { base.join(path_str) };
            match fs::read(&abs) {
                Ok(bytes) => {
                    if is_dq {
...
fn encode_operand(s: &str, labels: &HashMap<String, usize>, data_labels: &HashMap<String, usize>, reg_map: &HashMap<String, u16>) -> (u8, u64) {
    ...
    if let Some(&off) = labels.get(s) { return (2, off as u64); }
...
pub fn assemble_to_masi(src: &str) -> Result<Vec<u8>, String> {
    ...
    for ins in &insts {
        match ins {
            ...
            Instruction::Jmp(_) | Instruction::Je(_) | Instruction::Jne(_) | Instruction::Jl(_) | Instruction::Jle(_) | Instruction::Jg(_) | Instruction::Jge(_) | Instruction::Call(_)
            | Instruction::FJe(_) | Instruction::FJne(_) | Instruction::FJlt(_) | Instruction::FJle(_)
            | Instruction::FJgt(_) | Instruction::FJge(_) | Instruction::FJuo(_) => { if entry.is_none() { entry = Some(pc); } pc += 1 + 1 + 8; },
            Instruction::Push(_) | Instruction::Pop(_) | Instruction::In(_) => { if entry.is_none() { entry = Some(pc); } pc += 1 + 1 + 8; },
            Instruction::Out(_, _) | Instruction::COut(_, _) | Instruction::Mov(_, _) | Instruction::Add(_, _) | Instruction::Sub(_, _) | Instruction::Cmp(_, _)
            | Instruction::FMov(_, _) | Instruction::FAdd(_, _) | Instruction::FSub(_, _) | Instruction::FMul(_, _) | Instruction::FDiv(_, _) | Instruction::FCmp(_, _) => { if entry.is_none() { entry = Some(pc); } pc += 1 + (1+8) + (1+8); },
