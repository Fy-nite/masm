...
fn read_u16_le(data: &[u8], off: &mut usize) -> u16 { let mut v = 0u16; v |= data[*off + 0] as u16; v |= (data[*off + 1] as u16) << 8; *off += 2; v }
...
pub fn load(path: &str) -> Result<MASIFile, String> {
    ...
    if label_table.len() >= 2 {
        ...
        for _ in 0..count {
            if lt_off + 2 > label_table.len() { break; }
            let name_len = read_u16_le(&label_table, &mut lt_off) as usize;
            if lt_off + name_len + 8 > label_table.len() { break; }
            let name = String::from_utf8(label_table[lt_off..lt_off+name_len].to_vec()).unwrap_or_else(|_| "label_?".into());
            lt_off += name_len;
    ...
    if local_var_table.len() >= 2 {
        ...
        for _ in 0..count {
            if dl_off + 2 > local_var_table.len() { break; }
            let name_len = read_u16_le(&local_var_table, &mut dl_off) as usize;
            if dl_off + name_len + 8 > local_var_table.len() { break; }
            let name = String::from_utf8(local_var_table[dl_off..dl_off+name_len].to_vec()).unwrap_or_else(|_| "data_?".into());
            dl_off += name_len;
...
fn fmt_op(mode: u8, value: u64, masi: &MASIFile, reg_map_rev: &HashMap<u16, String>) -> String {
    match mode {
        ...
        1 => {
            let id = value as u16;
            if let Some(n) = reg_map_rev.get(&id) { n.clone() } else { format!("REG{}", id) }
        }
        2 => {
        ...
        4 => {
            let id = value as u16;
            if let Some(n) = reg_map_rev.get(&id) { format!("${}", n) } else { format!("$REG{}", id) }
        }
        _ => format!("{}", value),
    }
}
...
pub fn disassemble(masi: &MASIFile) -> String {
    ...
    while pc < code.len() {
        ...
        match opcode {
            ...
            0x31 => { let d=read_op(&mut pc); out.push(format!("POP {}", fmt_op(d.0,d.1,masi,&reg_rev))); }
            0x40 => { let p=read_op(&mut pc); let v=read_op(&mut pc); out.push(format!("OUT {} {}", fmt_op(p.0,p.1,masi,&reg_rev), fmt_op(v.0,v.1,masi,&reg_rev))); }
            0x41 => { let p=read_op(&mut pc); let v=read_op(&mut pc); out.push(format!("COUT {} {}", fmt_op(p.0,p.1,masi,&reg_rev), fmt_op(v.0,v.1,masi,&reg_rev))); }
            0x42 => { let d=read_op(&mut pc); out.push(format!("IN {}", fmt_op(d.0,d.1,masi,&reg_rev))); }
            0x50 => { let s=read_op(&mut pc); out.push(format!("ENTER {}", fmt_op(s.0,s.1,masi,&reg_rev))); }
            0x51 => { out.push("LEAVE".into()); }
            0x60 => { let m=read_op(&mut pc); let f=read_op(&mut pc); let argc = read_u16_le(code, &mut pc) as usize; let mut line = format!("MNI {} {}", fmt_op(m.0,m.1,masi,&reg_rev), fmt_op(f.0,f.1,masi,&reg_rev)); for _ in 0..argc { let a=read_op(&mut pc); line.push(' '); line.push_str(&fmt_op(a.0,a.1,masi,&reg_rev)); } out.push(line); }
            0x00 => { out.push("NOP".into()); }
            0xFF => { out.push("HLT".into()); }
            _ => { out.push(format!("; DB 0x{:02X} ; Unknown opcode", opcode)); }
        }
    }
    out.join("\n")
}
...
pub fn dump(masi: &MASIFile) -> String {
    ...
    for key in ["import","locals","labels","const","data","export","code"] { if let Some(sz) = masi.section_sizes.get(key) { lines.push(format!("  - {}: {} bytes", key, sz)); } }
    ...
    if masi.section_sizes.get("data").copied().unwrap_or(0) > 0 {
        ...
        for (off, name) in kv { lines.push(format!("  - 0x{:X}: {}", off, name)); }
        lines.push(format!("- Data section size: {} bytes", masi.section_sizes.get("data").unwrap()));
    }
    lines.join("\n")
}
