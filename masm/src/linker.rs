...
pub fn link_files(paths: &[String]) -> Result<LinkedOutput, String> {
    ...
    for p in paths {
        files.push(disassembler::load(p)?);
    }
    ...
    for (i, f) in files.iter().enumerate() {
        ...
        for im in &f.imports {
            if let Some(&(ei, off)) = export_map.get(&(im.kind, im.name.clone())) {
                let target_off = match im.kind {
                    0 => code_base[ei] as u64 + off,
                    1 => data_base[ei] as u64 + off,
                    _ => off,
                };
                for r in &im.refs {
                    ...
                    if r.section == 0 {
                        ...
                        if patch + 8 <= code.len() {
                            let bytes = target_off.to_le_bytes();
                            code[patch..patch + 8].copy_from_slice(&bytes);
                        }
                    }
                }
            } else {
    ...
    for (i, f) in files.iter().enumerate() {
        let base = code_base[i] as u64;
        for (off, name) in &f.label_map {
            label_entries.push((name.clone(), base + *off));
        }
    }
    write_u16_le(label_entries.len() as u16, &mut label_table);
    for (name, off) in label_entries {
        let nb = name.as_bytes();
        write_u16_le(nb.len() as u16, &mut label_table);
        label_table.extend_from_slice(nb);
        write_u64_le(off, &mut label_table);
    }
    ...
    for (i, f) in files.iter().enumerate() {
        let base = data_base[i] as u64;
        for (off, name) in &f.data_label_map {
            local_entries.push((name.clone(), base + *off));
        }
    }
    write_u16_le(local_entries.len() as u16, &mut locals);
    for (name, off) in local_entries {
        let nb = name.as_bytes();
        write_u16_le(nb.len() as u16, &mut locals);
        locals.extend_from_slice(nb);
        write_u64_le(off, &mut locals);
    }
    ...
    for (i, f) in files.iter().enumerate() {
        for e in &f.exports {
            let base = if e.kind == 0 {
                code_base[i]
            } else {
                data_base[i]
            } as u64;
            ex_entries.push((e.kind, e.name.clone(), base + e.offset));
        }
    }
    write_u16_le(ex_entries.len() as u16, &mut export_table);
    for (k, n, off) in ex_entries {
        export_table.push(k);
        let nb = n.as_bytes();
    ...
    let entry = if files.is_empty() {
        ...
        code_base[0] as u64 + files[0].entry
    };
    ...
    let mut chunk = |d: &[u8]| {
        write_u32_le(d.len() as u32, &mut out);
